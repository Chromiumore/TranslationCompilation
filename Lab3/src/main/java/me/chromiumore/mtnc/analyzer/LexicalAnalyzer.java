...
public class LexicalAnalyzer {
    ...
    public void analyze() {
        while (position < input.length()) {
            ...
            if (Character.isWhitespace(current)) {
                ...
                tokens.add(new Token(TokenType.DELIMITER, String.valueOf(current), position));
                position++;
            } else {
                String errorMsg = "Недопустимый символ '" + current + "' на позиции " + position;
                errors.add(errorMsg);
                tokens.add(new Token(TokenType.ERROR, String.valueOf(current), position));
                position++;
            }
        }
    }
    ...
    private void parseIdentifierOrKeyword() {
        ...
        if (KEYWORDS.contains(lexeme)) {
            ...
            errors.add(errorMsg);
            tokens.add(new Token(TokenType.ERROR, lexeme, position));
        } else {
    ...
    private void parseNumber() {
        ...
        if (position < input.length() && input.charAt(position) == '.') {
            ...
            if (position < input.length() && input.charAt(position) == '.') {
                ...
                if (position < input.length() && Character.isDigit(input.charAt(position))) {
                    ...
                    errors.add(errorMsg);
                    tokens.add(new Token(TokenType.ERROR, input.substring(start, position), position));
                    return;
                }
            }
        }
        ...
        if (position < input.length() && Character.isLetter(input.charAt(position))) {
            ...
            errors.add(errorMsg);
            tokens.add(new Token(TokenType.ERROR, lexeme + input.charAt(position), position));
            position++;
            return;
        }

        if (isReal) {
            tokens.add(new Token(TokenType.CONSTANT_REAL, lexeme, position));
        } else {
            tokens.add(new Token(TokenType.CONSTANT_INT, lexeme, position));
        }
    }
    ...
    private void parseString() {
        ...
        if (!closed) {
            ...
            errors.add(errorMsg);
            tokens.add(new Token(TokenType.ERROR, input.substring(start - 1), position));
            return;
        }

        String lexeme = input.substring(start - 1, position + 1);
        tokens.add(new Token(TokenType.CONSTANT_STRING, lexeme, position));
        position++;
    }
    ...
    private void parseOperator() {
        ...
        if (possibleOp != null) {
            tokens.add(new Token(TokenType.OPERATOR, possibleOp, position));
        } else {
            char c = input.charAt(position);
            String errorMsg = "Неизвестный оператор '" + c + "' на позиции " + position;
            errors.add(errorMsg);
            tokens.add(new Token(TokenType.ERROR, String.valueOf(c), position));
            position++;
        }
    }
